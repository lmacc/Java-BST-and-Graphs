...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		if(level == 1) {
			...
			res = this.my_left_tree().my_count_at_level(level - 1) + this.my_right_tree().my_count_at_level(level -1);
	...
	public boolean my_is_balanced(){
		...
		if(this.my_is_empty())
			...
			if(Math.abs(l1 - l2) <= 1){
				boolean leftBalanced = this.my_left_tree().my_is_balanced();
				boolean rightBalanced = this.my_right_tree().my_is_balanced();
	...
	public int my_count_smaller_nodes(T1 key){
		...
		if (this.root == null)
			...
			int k = this.my_root().getKey().compareTo(key);
			if (k < 0) {
				res += 1;

				res += this.my_left_tree().my_node_count();
